function HyperEF(ar, L, R, abc)
	...
	@inbounds for loop ∈ 1:L
		...
		if loop == 1
			Rinit = Evec
			Rinit = ((Rinit ./ maximum(Rinit)) .* 20) .+ 2
			open("$abc.ER", "w") do io
				for ii in 1:length(Rinit)
					println(io, Rinit[ii])
				end
			end
		end
	...
	# println("done")
	# /home/CAMPUS/hsajadin/cuda11/cuda-samples-11.8/Samples/New/HyperEF/data/ibm01.hgr
end
