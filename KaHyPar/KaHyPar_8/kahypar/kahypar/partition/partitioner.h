...
namespace kahypar
{
    ...
    static inline void partition(Hypergraph &hypergraph, const Context &context)
    {
      ...
      else
      {
        ...
        case Mode::recursive_bisection:
          ...
          {
            if (hypergraph.edgeSize(he) > Maxxx)
            {
              Maxxx = hypergraph.edgeSize(he); // Update max_value if a larger value is found
            }
            if (numberss[he] > Max_Reff)
            {
              Max_Reff = numberss[he]; // Update max_value if a larger value is found
            }
          }
          Ratio_Reff = Maxxx / Max_Reff;
  ...
  public:
    Partitioner() : _single_node_he_remover(),
                    _large_he_remover(),
                    _pin_sparsifier(),
                    _deduplicator() {}
  ...
  inline void Partitioner::setupContext(const Hypergraph &hypergraph, Context &context)
  {
    ...
    context.coarsening.max_allowed_node_weight = ceil(context.coarsening.hypernode_weight_fraction * hypergraph.totalWeight());
  ...
  inline void Partitioner::sanitize(Hypergraph &hypergraph, const Context &context)
  {
        ...
        (result.num_removed_single_node_hes > 0 || num_removed_large_hes > 0))
    {
      ...
      LOG << "\033[1m\033[31m"
          << " # removed hyperedges with |e|=1 = "
          << result.num_removed_single_node_hes
          << "\033[0m";
      LOG << "\033[1m\033[31m"
          << " ===>" << result.num_unconnected_hns
          << "unconnected HNs could have been removed"
          << "\033[0m";
      LOG << "Performing large HE removal:";
      LOG << "\033[1m\033[31m"
          << " # removed hyperedges with |e|>"
          << context.partition.max_he_size_threshold << " = "
