...
std::vector<float> numberss;
std::vector<float> community_numbers;
unsigned int Maxxx;
float Max_Reff;
float Ratio_Reff;
 ...
 */
  ...
  */
  ...
  /*void copyFromVector(const std::vector<int>& source) {
        vec = source; // This copies all elements from source to vec
    }*/
  HypernodeID edgeResistance(const HyperedgeID e) const {
    ASSERT(!hyperedge(e).isDisabled(), "Hyperedge" << e << "is disabled");
    return numberss[e];
  }
